...
export default function Navigation() {
  return (
    <nav className='col-span-1 flex flex-col items-start justify-between
      ...
      <ul className='flex w-full flex-col items-start justify-between gap-2'>
        {['patients', 'customers', 'inventory'].map((element) => (
          <Link href={`/dashboard/${element}`} key={element} className='w-full'>
            <li className='w-full rounded bg-zinc-700 px-4 py-2 capitalize border-2 border-zinc-600 hover:bg-zinc-800 transition-colors delay-[50ms]'>{element}</li>
          </Link>
        ))}
      </ul>
    </nav>
